...
# CORE LOGIC
# ============================================================================
def parse_marketplace_file(file) -> pd.DataFrame:
...
def build_comparison(marketplace_df, warehouse_df, platform_df) -> pd.DataFrame:
    ...
    df["Overall_Status"] = np.where(df["Discrepancy_Count"] == 0, "OK", "Discrepancy")
    ...
    df = df[[
        ...
        "Discrepancy_Count", "Overall_Status",
    ]]
...
with st.expander("ℹ️ How this works", expanded=False):
    st.markdown(
        """
        1. **SKU lookup** always comes from the Marketplace file — column **F (SKU)**;
           if that's blank, column **E (Parent SKU)** is used instead.
...
with col1:
    marketplace_file = st.file_uploader(
        "🛒 Marketplace file (e.g. Shopee)", type=["xlsx", "xls"], key="mp"
    )
with col2:
...
if run:
    ...
    try:
        with st.spinner("Parsing files..."):
            mp_df = parse_marketplace_file(marketplace_file)
        ...
        with st.spinner("Comparing stock..."):
            comparison = build_comparison(mp_df, wh_df, plt_df)
...
if "comparison" in st.session_state:
    ...
    m4.metric("Marketplace mismatches", int(mp_mismatch))
    m5.metric("Platform mismatches", int(plt_mismatch))
    ...
    with tab1:
        ...
        if len(result):
            st.dataframe(
                result.style.applymap(style_status, subset=["Marketplace_Status", "Platform_Status", "Overall_Status"]),
    ...
    with tab2:
        ...
        st.dataframe(
            comparison.style.applymap(style_status, subset=["Marketplace_Status", "Platform_Status", "Overall_Status"]),
            use_container_width=True,
            height=500,
        )
        st.download_button(
